...
def test_audio_conversion_service_passthrough_for_wav(monkeypatch, tmp_path):
    ...
    run_mock = Mock()
    replace_mock = Mock(wraps=audio_module.os.replace)
    monkeypatch.setattr(audio_module.subprocess, "run", run_mock)
    monkeypatch.setattr(audio_module.os, "replace", replace_mock)
    ...
    assert output_path.read_text() == "raw-audio"
    assert not input_path.exists()
    run_mock.assert_not_called()
    replace_mock.assert_called_once_with(str(input_path), str(output_path))


def test_audio_conversion_service_converts_mp3(monkeypatch, tmp_path):
    import pyutube.services.AudioConversionService as audio_module
