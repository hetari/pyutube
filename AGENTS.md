...
- `pyutube/tests/` contains pytest-based tests.
...
- `pytest pyutube/tests` runs the test suite.
...
- Prefer `snake_case` for functions, variables, and test names; keep module names aligned with their purpose, such as `DownloadService.py` and `URLHandler.py`.
...
## Testing Guidelines
- Tests currently use `pytest` and live under `pyutube/tests/`.
- Name tests with clear intent, such as `test_is_youtube_link`.
- Add parameterized cases for URL parsing and downloader edge cases when possible.
- Run `pytest pyutube/tests` before opening a pull request, especially after touching URL validation or download flow code.
...
- PRs should include a short summary, the user-visible impact, and any test results.
